...
3. Zhang 下一步从尺寸分组候选升级到更高保真校准：以 C `44.772/0.654`、D `37.517/0.77`、E `37.517/0.693` 为起点，`particle_count_scale=2` 的三条 GitHub demo 已全部跑通并导入；随后 9-job pressure-first 重标定也已跑通并导入。当前结论是 C 接近窗口但略低，D 压力回窗但 trend review，E 仍偏低，因此下一步不是 4x/8x，而是运行 `zhang-pscale2-followup.yml`：C 窄扫 Emax，D 固定压力候选调摩擦，E 延伸 endpoint modulus。继续使用 `allow_evidence_mismatch=true` 收集完整但非 pass 的样本，缺文件仍失败，趋势/压力不通过则进入 ensemble 诊断。
