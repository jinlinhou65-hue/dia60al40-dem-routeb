...
3. Zhang 下一步从尺寸分组候选升级到更高保真校准：低侧壁摩擦已被固定求解器配对试验否决，恢复 C settle scale 1、50 cm/s 和 wall scale 1。下一轮先把粒间摩擦与压头摩擦分离，预注册 particle-only 单变量试验；禁止把论文阻尼 `0.2` 直接当作恢复系数，D/E 和 4x/8x 暂不推进。
