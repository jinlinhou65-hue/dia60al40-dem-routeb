...
3. Zhang 下一步从尺寸分组候选升级到更高保真校准：低侧壁和低粒间摩擦都已被固定求解器配对试验否决，冻结 C baseline。E 组五种子 `Emax=96.417 GPa` pressure-lift 已预注册，下一步运行并按门槛判读；禁止把论文阻尼 `0.2` 直接当作恢复系数，4x/8x 暂不推进。
