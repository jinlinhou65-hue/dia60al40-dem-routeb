...
class PaperReproductionTest(unittest.TestCase):
    ...
    def test_render_dem_deck_supports_fast_demo_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("top_velocity_cm_s,50,cm/s", text)
            self.assertIn("time_step_seconds,2e-10,s", text)
            self.assertIn("initial_settle_steps,20,step", text)
            self.assertIn("stage_settle_steps,30,step", text)
            self.assertIn("final_settle_steps,40,step", text)
    ...
    def test_real_dem_workflow_runs_paper_stage_series_postprocess(self):
        ...
        self.assertIn("--ds-radius-um", workflow)
        self.assertIn("--dl-radius-um", workflow)
