"""Summarize imported Zhang pscale=2 follow-up DEM results."""
...
def analyze_followup(
    *,
    ...
    for run_id in run_ids:
        run_dir = evidence_root / f"pscale2_followup_run_{run_id}"
        run_rows = read_csv(run_dir / "zhang_calibration_best_by_run.csv")
        if not run_rows:
            raise SystemExit(f"[FAIL] missing follow-up rows for run {run_id}: {run_dir}")
        for row in run_rows:
            rows.append(normalize_row(row, run_id=run_id, run_dir=run_dir))

    outdir.mkdir(parents=True, exist_ok=True)
    figure_dir.mkdir(parents=True, exist_ok=True)
    write_csv(outdir / "pscale2_followup_candidates.csv", rows)
    size_rows = summarize_by_size(rows)
    write_csv(outdir / "pscale2_followup_size_summary.csv", size_rows)
    acceptance_rows = [acceptance_row(row) for row in size_rows]
    write_csv(outdir / "pscale2_followup_acceptance.csv", acceptance_rows)
    summary = {
        "run_ids": run_ids,
        "row_count": len(rows),
    ...
    (outdir / "pscale2_followup_summary.json").write_text(
...
def candidate_sort_key(row: dict[str, object]) -> tuple[int, int, float, int, float]:
    ...
    participation = optional_float(row.get("participation_delta"))
    return (
        0 if row["pressure_in_window"] else 1,
        0 if row["status"] == "pass" else 1,
        abs(p95 - 600.0),
        0 if participation is not None and participation > 0 else 1,
...
def decision_for_size(size: str, group: list[dict[str, object]], best: dict[str, object]) -> str:
    ...
    if best["pressure_in_window"]:
        return "hold_pressure_candidate_and_tune_trend"
    max_pressure = max(float(row["p95_mpa"]) for row in group)
    has_pass = any(row["status"] == "pass" for row in group)
    has_positive_participation = any(
        optional_float(row.get("participation_delta")) is not None
        and float(row["participation_delta"]) > 0
        for row in group
    )
    if size == "D" and has_positive_participation:
        return "decouple_participation_from_pressure_and_d1"
    if has_pass and max_pressure < ZHANG_WINDOW[0]:
        return "extend_pressure_without_losing_trend"
...
def recommend_next_step(size_rows: list[dict[str, object]]) -> str:
    ready = [
        row["diamond_size_case"]
        for row in size_rows
        if row["best_pressure_in_window"] and row["best_status"] == "pass"
    ]
    if len(ready) == len(size_rows):
        return "All C/D/E pscale=2 candidates are ready for a seed robustness recheck before 4x."
    return (
        "C now has a pscale=2 pressure-window and trend-pass candidate, but D and E are not ready "
        "for 4x. Hold C for a seed recheck; for D, pressure and force-chain trend conflict under "
        "friction-only tuning; for E, pressure improved but remains below the 572 MPa lower bound."
    )
...
def write_report(
    ...
    *,
    ...
    for row in size_rows:
        lines.append(
            "| {size} | {emax:.3f} | {mu:.3f} | {p95:.3f} | {pressure} | {trend} | {part} | {d1} | {decision} |".format(
                size=row["diamond_size_case"],
                emax=float(row["best_e_al_emax_gpa"]),
                mu=float(row["best_mu_scale"]),
                ...
                d1=format_number(row["best_d1_delta"]),
                decision=row["decision"],
            )
        )
    lines.extend(
...
def write_plots(rows: list[dict[str, object]], *, figure_dir: Path) -> None:
    try:
        ...
        (figure_dir / "pscale2_followup_plots_skipped.txt").write_text(
    ...
    for size, group in sorted(grouped.items()):
        ordered = sorted(group, key=lambda item: (float(item["e_al_emax_gpa"]), float(item["mu_scale"])))
        labels = [
            f'E={float(item["e_al_emax_gpa"]):.3f}, mu={float(item["mu_scale"]):.3f}'
            for item in ordered
        ]
        ax.plot(
            range(len(ordered)),
        ...
        for index, label in enumerate(labels):
            ax.annotate(label, (index, float(ordered[index]["p95_mpa"])), fontsize=7, xytext=(2, 4), textcoords="offset points")
    ax.axhspan(ZHANG_WINDOW[0], ZHANG_WINDOW[1], color="#d8ead3", alpha=0.45, label="Zhang 572-638 MPa")
    ax.set_xlabel("Candidate index within size case")
    ax.set_ylabel("P95 pressure (MPa)")
    ax.set_title("Zhang pscale=2 follow-up pressure results")
    ax.legend()
    fig.tight_layout()
    fig.savefig(figure_dir / "pscale2_followup_p95.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--run-ids-json", default='["27883033303","27883034434","27883035658"]')
    ...
    parser.add_argument("--report", default="docs/reproduction_goal/03_zhang_particle_scale/pscale2_followup_report.md")
